...
export default {
	async fetch(request, env, ctx) {
		try {
			...
			if (!userID) {
				return new Response('Please: 1.setup your UUID variable 2.try to redeploy 3.check the variable are vaild？', {
					status: 404,
					headers: {
			...
			if (!upgradeHeader || upgradeHeader !== 'websocket') {
				...
				if (路径 == '/') {
					...
					else return new Response('The UUID is definitely wrong.', { status: 404 });
				}
			} else {
...
async function 维列斯OverWSHandler(request) {
	...
	readableWebSocketStream.pipeTo(new WritableStream({
		async write(chunk, controller) {
			...
			if (isUDP) {
				if (portRemote === 53) {
					...
					throw new Error('UDP proxy is only on in DNS(port 53)');
					return;
				}
			}
			// 构建 维列斯 响应头部
			...
			if (!banHosts.includes(addressRemote)) {
				log(`handle TCP out connection ${addressRemote}:${portRemote}`);
				handleTCPOutBound(remoteSocketWapper, addressType, addressRemote, portRemote, rawClientData, webSocket, 维列斯ResponseHeader, log);
			} else {
				throw new Error(`BLocklist is turned off in TCP out connection ${addressRemote}:${portRemote}`);
			}
		},
		close() {
		...
		abort(reason) {
			log(`readableWebSocketStream stopped`, JSON.stringify(reason));
		},
	})).catch((err) => {
		log('readableWebSocketStream wrong tunnel', err);
	});
	...
	const stream = new ReadableStream({
		...
		start(controller) {
			...
			webSocketServer.addEventListener('error', (err) => {
				log('WebSocket server error occurred');
				// 将错误传递给控制器
		...
		cancel(reason) {
			...
			log(`Readable stream is cancelled，because ${reason}`);
...
function stringify(arr, offset = 0) {
	...
	if (!isValidUUID(uuid)) {
		...
		throw TypeError(`The UUID's format is incrrect: ${uuid}`);
		//uuid = userID;
	}
	return uuid;
}
...
async function handleDNSQuery(udpChunk, webSocket, 维列斯ResponseHeader, log) {
	...
	try {
		...
		await tcpSocket.readable.pipeTo(new WritableStream({
			...
			close() {
				log(`The DNS server(${dnsServer}) TCP connection closed`); // 记录连接关闭信息
			},
			abort(reason) {
				console.error(`The DNS server(${dnsServer})'s TCP connection closed unexpected`, reason); // 记录异常中断原因
			},
		}));
	} catch (error) {
		// 捕获并记录任何可能发生的错误
		console.error(
			`handleDNSQuery function was got a error, log: ${error.message}`
		);
	}
}
...
async function socks5Connect(addressType, addressRemote, portRemote, log) {
	...
	log('SOCKS5 greeting message sended');
	...
	// +----+--------+
	if (res[0] !== 0x05) {
		log(`SOCKS5 server version incorrect: got ${res[0]}, expected 5`);
		return;
	}
	if (res[1] === 0xff) {
		log("Server doesn't accept any verify method");
		return;
	}

	// 如果返回 0x0502，表示需要用户名/密码认证
	if (res[1] === 0x02) {
		log("SOCKS5 server needs a verification");
		...
		if (res[0] !== 0x01 || res[1] !== 0x00) {
			log("SOCKS5 server verification failed");
			return;
		}
	}
	...
	switch (addressType) {
		...
		case 3: // IPv6
			...
			break;
		default:
			log(`invaild address type: ${addressType}`);
			return;
	}
...
async function httpConnect(addressRemote, portRemote, log) {
	...
	log(`connecting to ${addressRemote}:${portRemote} via ${hostname}:${port}`);
	...
	try {
		...
		console.error('HTTP CONNECT request fail to send:', err);
		throw new Error(`HTTP CONNECT request fail to send: ${err.message}`);
	}
	...
	try {
		while (true) {
			...
			if (done) {
				console.error('HTTP proxy connection closed');
				throw new Error('HTTPproxy connection closed');
			}
			...
			if (respText.includes('\r\n\r\n')) {
				...
				log(`Received HTTP proxy message: ${headers.split('\r\n')[0]}`);
				...
				if (headers.startsWith('HTTP/1.1 200') || headers.startsWith('HTTP/1.0 200')) {
					...
					if (headersEndPos < responseBuffer.length) {
						...
						dataStream.pipeTo(writable).catch(err => console.error('handle lasted data errors:', err));
					...
					const errorMsg = `HTTP proxy connect failed: ${headers.split('\r\n')[0]}`;
...
async function 生成配置信息(userID, hostName, sub, UA, RproxyIP, _url, fakeUserID, fakeHostName, env) {
		...
		if ((addresses.length + addressesapi.length + addressesnotls.length + addressesnotlsapi.length + addressescsv.length) == 0) {
			...
			if (hostName.includes("worker") || hostName.includes("notls")) {
				...
				addressesnotls = addressesnotls.concat(
					cfips.map(cidr => generateRandomIPFromCIDR(cidr) + ':' + randomPorts[Math.floor(Math.random() * randomPorts.length)] + '#CF Random Node' + String(counter++).padStart(2, '0'))
				);
			} else {
	...
	if (userAgent.includes('mozilla') && !subParams.some(_searchParams => _url.searchParams.has(_searchParams))) {
		...
		if (sub) {
			...
			else if (RproxyIP == 'true') 订阅器 += `CFCDN（访问方式）: Autoget ProxyIP<br>`;
			else 订阅器 += `CFCDN（访问方式）: cannot access, you may need to setup proxyIP/PROXYIP.<br>`
			订阅器 += `<br>SUB（Subscription Generator）: ${sub}`;
		} else {
			if (enableSocks) 订阅器 += `CFCDN（访问方式）: ${enableHttp ? "HTTP" : "Socks5"}<br>&nbsp;&nbsp;${newSocks5s.join('<br>&nbsp;&nbsp;')}<br>${socks5List}`;
			else if (proxyIP && proxyIP != '') 订阅器 += `CFCDN（访问方式）: ProxyIP<br>&nbsp;&nbsp;${proxyIPs.join('<br>&nbsp;&nbsp;')}<br>`;
			else 订阅器 += `CFCDN（访问方式）: cannot access, you may need to setup proxyIP/PROXYIP.<br>`;
			...
			订阅器 += `<br>Your subscription is provided by built-in addresses/ADD* ${判断是否绑定KV空间}<br>`;
		...
		else 订阅器 += `<br>SUBAPI（Sub => backside）: ${subProtocol}://${subConverter}<br>SUBCONFIG（Sub => config file）: ${subConfig}`;
		const 动态UUID信息 = (uuid != userID) ? `TOKEN: ${uuid}<br>UUIDNow: ${userID}<br>UUIDLow: ${userIDLow}<br>${userIDTime}TIME（Dynamic UUID vaild time）: ${有效时间} 天<br>UPTIME（Dynamic UUID update time）: ${更新时间} O' clock（CST）<br><br>` : `${userIDTime}`;
		const 节点配置页 = `
			################################################################<br>
			Subscribe / sub, click to <strong>Copy sub link</strong> and <strong>Generate QR code</strong> automaticly. <br>
			---------------------------------------------------------------<br>
			Auto Subscribe Link:<br>
			...
			<strong><a href="javascript:void(0);" id="noticeToggle" onclick="toggleNotice()">Useful Subscribe Tips∨</a></strong><br>
				<div id="noticeContent" class="notice-content" style="display: none;">
					<strong>1.</strong> If you are using PassWall, PassWall2 route addon, The user-agent which the subscription modified <strong>, set it to <strong>PassWall</strong>.<br>
					<br>
					<strong>2.</strong> If you are using SRR+ route addon, <strong>Base64 Subscribe Link</strong> is recommended；<br>
					<br>
					<strong>3.</strong> Quickly switch <a href='${atob('aHR0cHM6Ly9naXRodWIuY29tL2NtbGl1L1dvcmtlclZsZXNzMnN1Yg==')}'>Subscription Generator</a> to：sub.google.com，you can add"?sub=sub.google.com"to the end of the link, e.g.:<br>
					&nbsp;&nbsp;https://${proxyhost}${hostName}/${uuid}<strong>?sub=sub.google.com</strong><br>
					<br>
					<strong>4.</strong> Quickly switch PROXYIP to：proxyip.cmliussss.net:443，you can add"?proxyip=proxyip.cmliussss.net:443"to the end of the link, e.g.:<br>
					&nbsp;&nbsp; https://${proxyhost}${hostName}/${uuid}<strong>?proxyip=proxyip.cmliussss.net:443</strong><br>
					<br>
					<strong>5.</strong> Quickly switch SOCKS5 to: user:password@127.0.0.1:1080, you can add"?socks5=user:password@127.0.0.1:1080"to the end of the link, e.g.:<br>
					&nbsp;&nbsp;https://${proxyhost}${hostName}/${uuid}<strong>?socks5=user:password@127.0.0.1:1080</strong><br>
					<br>
					<strong>6.</strong> If there are more than one index then they should devided with'&',e.g.:<br>
			...
			function toggleNotice() {
				...
				if (noticeContent.style.display === 'none') {
					...
					noticeToggle.textContent = 'Useful Subscribe Tips∧';
				} else {
					noticeContent.style.display = 'none'; 
					noticeToggle.textContent = 'Useful Subscibe Tips∨';
				}
			}
			</script>
			---------------------------------------------------------------<br>
			...
			FKID: ${fakeUserID}<br>
			UA: ${UA}<br>
			SCV（Bypass TLS Licence verification）: ${SCV}<br>
			${订阅器}<br>
			---------------------------------------------------------------<br>
			################################################################<br>
			v2ray<br>
...
async function KV(request, env, txt = 'ADD.txt') {
	try {
		...
		const html = `
			...
			<body>
				################################################################<br>
				${FileName} Subscribe Generator:<br>
				---------------------------------------------------------------<br>
				...
				<div class="editor-container">
					...
					<div class="save-container">
						<button class="back-btn" onclick="goBack()">Back to config</button>
						<button class="save-btn" onclick="saveContent(this)">Save</button>
						<span class="save-status" id="saveStatus"></span>
					</div>
					<br>
					################################################################<br>
					` : '<p>KV Namespace Unbound</p>'}
				</div>
