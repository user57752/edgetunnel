...
async function 生成配置信息(userID, hostName, sub, UA, RproxyIP, _url, fakeUserID, fakeHostName, env) {
	if (sub) {
		...
		if ((addresses.length + addressesapi.length + addressesnotls.length + addressesnotlsapi.length + addressescsv.length) == 0) {
			...
			if (hostName.includes(".workers.dev")) {
				addressesnotls = addressesnotls.concat(cfips.map(cidr => generateRandomIPFromCIDR(cidr) + '#CF Random Node'));
			} else {
				addresses = addresses.concat(cfips.map(cidr => generateRandomIPFromCIDR(cidr) + '#CF Random Node'));
			}
		}
	}
	...
	if (userAgent.includes('mozilla') && !subParams.some(_searchParams => _url.searchParams.has(_searchParams))) {
		...
		if (sub) {
			...
			else if (RproxyIP == 'true') 订阅器 += `CFCDN（Access Method）: 自动获取ProxyIP<br>`;
			else 订阅器 += `CFCDN（Access Method）: Unable to access, you need to set up proxyIP/PROXYIP ！！！<br>`
			订阅器 += `<br>SUB（优选订阅生成器）: ${sub}`;
		} else {
			if (enableSocks) 订阅器 += `CFCDN（Access Method）: Socks5<br>&nbsp;&nbsp;${newSocks5s.join('<br>&nbsp;&nbsp;')}<br>${socks5List}`;
			else if (proxyIP && proxyIP != '') 订阅器 += `CFCDN（Access Method）: ProxyIP<br>&nbsp;&nbsp;${proxyIPs.join('<br>&nbsp;&nbsp;')}<br>`;
			else 订阅器 += `CFCDN（Access Method）: Unable to access, you need to set up proxyIP/PROXYIP ！！！<br>`;
			let 判断是否绑定KV空间 = '';
			if (env.KV) 判断是否绑定KV空间 = ` <a href='${_url.pathname}/edit'>编辑优选列表</a>`;
			订阅器 += `<br>Your Subscribe content is from Built-in addresses/ADD* parameter variable${判断是否绑定KV空间}<br>`;
			if (addresses.length > 0) 订阅器 += `ADD（TLS preferred domain name&IP）: <br>&nbsp;&nbsp;${addresses.join('<br>&nbsp;&nbsp;')}<br>`;
			if (addressesnotls.length > 0) 订阅器 += `ADDNOTLS（noTLS preferred domain name&IP）: <br>&nbsp;&nbsp;${addressesnotls.join('<br>&nbsp;&nbsp;')}<br>`;
			if (addressesapi.length > 0) 订阅器 += `ADDAPI（TLS preferred domain &IP 's API）: <br>&nbsp;&nbsp;${addressesapi.join('<br>&nbsp;&nbsp;')}<br>`;
			if (addressesnotlsapi.length > 0) 订阅器 += `ADDNOTLSAPI（noTLS preferred domain &IP 's API）: <br>&nbsp;&nbsp;${addressesnotlsapi.join('<br>&nbsp;&nbsp;')}<br>`;
			if (addressescsv.length > 0) 订阅器 += `ADDCSV（IPTest测速csv文件 限速 ${DLS} ）: <br>&nbsp;&nbsp;${addressescsv.join('<br>&nbsp;&nbsp;')}<br>`;
		}

		if (动态UUID && _url.pathname !== `/${动态UUID}`) 订阅器 = '';
		else 订阅器 += `<br>SUBAPI（Subscribe -> Back end）: ${subProtocol}://${subConverter}<br>SUBCONFIG（Subscribe -> Config file）: ${subConfig}`;
		const 动态UUID信息 = (uuid != userID) ? `TOKEN: ${uuid}<br>UUIDNow: ${userID}<br>UUIDLow: ${userIDLow}<br>${userIDTime}TIME（Dynamic UUID validity time）: ${有效时间} Days<br>UPTIME（Dynamic UUID validity time）: ${更新时间} o'clock（Beijing Time）<br><br>` : `${userIDTime}`;
		const 节点配置页 = `
			Welcome Back user57752!<br>
			################################################################<br>
			Subscribe / sub | Click the link to <strong>copy the subscribe link</strong> and will <strong>generate a QR code automaticly</strong>. <br>
			---------------------------------------------------------------<br>
				...
				if (document.querySelector('.editor')) {
					...
					function saveContent(button) {
						try {
							...
							const textarea = document.getElementById('content');
							if (!textarea) {
								throw new Error('Unable to find text area');
							}
